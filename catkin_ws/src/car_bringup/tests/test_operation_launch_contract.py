...
class OperationLaunchContractTests(unittest.TestCase):
    ...
    def test_start_mapping_launch_starts_stack_and_recording(self):
        text = self.launch_text("start_mapping.launch")
        self.assertIn('name="map_id"', text)
        self.assertIn('name="start_stack" default="true"', text)
        self.assertIn('$(find car_bringup)/launch/manual_mapping.launch', text)
    ...
    def test_relocalization_launch_starts_localization_without_motion(self):
        text = self.launch_text("start_relocalization.launch")
        self.assertIn('name="map_id"', text)
        self.assertIn('$(find car_bringup)/launch/autonomy.launch', text)
        self.assertIn('name="start_navigation" value="false"', text)
        self.assertIn('name="start_control" value="false"', text)
        self.assertIn('name="start_mission" value="false"', text)
