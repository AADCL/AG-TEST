...
| `start_fastlio_odometry_to_px4` | false | `/Odometry_loc` → `/mavros/odometry/out` |
...
| `laserMapping` | `fast_lio_open3d` | Livox 点云、IMU | 注册点云、`/Odometry_loc` | `camera_init → body` |
| `fastlio_odometry_to_px4` | `car_bringup` | `/Odometry_loc` | `/mavros/odometry/out` | 不发布 ROS TF |
| map recorder | `ground_air_mapping` | `/cloud_registered_1`、`/map` | mapping status 和三项服务 | 建图身份链由 TF owner 管理 |
...
| `/cloud_registered_1` | `sensor_msgs/PointCloud2` | FAST-LIO | 建图、重定位、平台 | `camera_init` | 已实机验证 |
| `/cloud_registered_body_1` | `sensor_msgs/PointCloud2` | FAST-LIO | costmap、融合客户端 | `body` | 避障已验证 |
| `/Odometry_loc` | `nav_msgs/Odometry` | FAST-LIO | TEB、重定位、适配器 | parent=`camera_init`、child=`body` | 已实机验证 |
...
rostopic hz /cloud_registered_body_1
...
| 接口 | 类型 | 作用 | 状态 |
|---|---|---|---|
| `/ground_air/mission/submit` | `ground_air_msgs/SubmitMission` | 提交有序 `PoseStamped[]` | 待整套实机验收 |
| `/ground_air/mission/start` | `std_srvs/Trigger` | 启动任务 | 待验收 |
| `/ground_air/mission/pause` | `std_srvs/Trigger` | 暂停并停止 | 待验收 |
| `/ground_air/mission/resume` | `std_srvs/Trigger` | 恢复 | 待验收 |
| `/ground_air/mission/cancel` | `std_srvs/Trigger` | 取消并停止 | 待验收 |
| `/ground_air/mission/status` | `ground_air_msgs/MissionStatus` | 任务状态 | 待验收 |
...
| `lukong_fusion_client` | `/Odometry_loc`、`/cloud_registered_body_1` 关键帧 HTTP 发送 | 待服务器联调 |
...
| obstacle topic | `/cloud_registered_body_1` |
...
2. `/cloud_registered_1` 新鲜且 frame=`camera_init`；
3. `/Odometry_loc` 新鲜；
4. 机器人静止；
5. 当前区域与历史地图有足够重叠；
6. 场景不应只有重复、对称结构。
